...
void setup() {
  size(3840, 2160); // ***CHANGE THIS TO YOUR SCREEN RESOLUTION***
  smooth();
...
void serialEvent(Serial myPort) { // starts reading data from the Serial Port
  ...
  angle = data.substring(0, index1); // read the data from position "0" to the position of the variable index1
...
void drawObject() {
  ...
  translate(width / 2, height - height * 0.074); // moves the starting coordinates to a new location
  noStroke();
  fill(255, 10, 10); // red color
  
  // Convert the distance from cm to pixels (adjusted for 400 cm max range)
  pixsDistance = iDistance * ((height - height * 0.1666) * 0.0025); // Adjusted for 400 cm
  
  // Limit the range to 400 cm
  if (iDistance < 400) {
    // Draw the object as a point according to the angle and distance
